...
int main(int argc, char* argv[])
{
    ...
    try {
        std::string prefix = "../../models/" + model_name + "_nids";
        std::string metadata_path = prefix + "_metadata.json";
        std::string model_path = prefix + ".onnx";
        std::string preprocessing_path = prefix + "_preprocessing_params.json";
        ...
        spdlog::info("Сервер {} запущен на http://{}:{}", model_name, host, port);
        drogon::app().run();

        uint64_t req_count = g_request_count.load();
        double avg_time = req_count ? g_total_inference_time / req_count : 0.0;
        if (req_count > 0) {
            json entry = {
                ...
                {"host", host},
                {"port", port},
                {"requests_total", (int64_t)req_count},
                {"inference_time_ms", {
                    {"mean", avg_time},
                    {"total_ms", g_total_inference_time}
                }}
            };
            ...
            if (!all["cpp"].contains(model_name)) all["cpp"][model_name] = json::array();
            all["cpp"][model_name].push_back(entry);
